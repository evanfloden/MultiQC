"""MultiQC submodule to parse output from Sentieon QualCal"""
...
class QualCalMixin:
    def parse_sentieon_qualcal(self):
        """Find Sentieon QualCal logs and parse their data"""

        report_table_headers = {
            "#:SENTIEON_QCAL_TABLE:Arguments:Recalibration argument collection values used in this run": "arguments",
            "#:SENTIEON_QCAL_TABLE:Quantized:Quality quantization map": "quality_quantization_map",
            "#:SENTIEON_QCAL_TABLE:RecalTable0:": "recal_table_0",
            "#:SENTIEON_QCAL_TABLE:RecalTable1:": "recal_table_1",
            "#:SENTIEON_QCAL_TABLE:RecalTable2:": "recal_table_2",
        }
        samples_kept = {rt_type: set() for rt_type in recal_table_type}
        self.sentieon_qualcal = {
            recal_type: {table_name: {} for table_name in report_table_headers.values()}
            for recal_type in recal_table_type
        }

        for f in self.find_log_files("sentieon/qualcal", filehandles=True):
            ...
            if len(parsed_data) > 0:
                ...
                self.add_data_source(f, section="qualcal")
                for table_name, sample_tables in parsed_data.items():
                    self.sentieon_qualcal[rt_type][table_name][f["s_name"]] = sample_tables

        # Filter to strip out ignored sample names. Again.
        for rt_type in recal_table_type:
            for table_name, sample_tables in self.sentieon_qualcal[rt_type].items():
                self.sentieon_qualcal[rt_type][table_name] = self.ignore_samples(sample_tables)

        n_reports_found = sum([len(samples_kept[rt_type]) for rt_type in recal_table_type])

        if n_reports_found > 0:
            log.info(f"Found {n_reports_found} QualCal reports")

            # Write data to file
            self.write_data_file(self.sentieon_qualcal, "sentieon_qualcal")
    ...
    def add_quality_score_vs_no_of_observations_section(self):
        ...
        for rt_type_name, rt_type in recal_table_type._asdict().items():
            sample_tables = self.sentieon_qualcal[rt_type]["quality_quantization_map"]
        ...
        plot = linegraph.plot(
            sample_data,
            pconfig={
                "title": "Sentieon: Observed Quality Score Counts",
                "id": "sentieon-qualcal-quality-scores-plot",
        ...
        self.add_section(
            ...
            anchor="sentieon-qualcal-quality-scores",
            description=(
    ...
    def add_reported_vs_empirical_section(self):
        ...
        for (
            ...
            sample_tables = self.sentieon_qualcal[rt_type]["recal_table_1"]
            if len(sample_tables) == 0:
                continue
        ...
        plot = scatter.plot(
            ...
            pconfig={
                ...
                "id": "sentieon-qualcal-reported-empirical-plot",
        ...
        self.add_section(
            ...
            anchor="sentieon-qualcal-reported-empirical",
